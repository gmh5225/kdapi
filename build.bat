...
SET "KDAPI_SOURCE_FILES=test.c"
SET KDAPI_OUTPUT_NAME=test

GOTO :_KDAPI_START_BUILDING
...
:_KDAPI_ENVIRONMENT_INFO
  ...
  ECHO %TAB%- KDAPI_LINKER_NAME ^(optional^) ^(i.e. ld.exe, clang-ld.exe^)

  ECHO %TAB%- KDAPI_COMPILER_OPTIMIZE
  ECHO %TAB% %TAB% Valid values:
  ECHO %TAB% %TAB% %TAB% true false
...
IF "%KDAPI_COMPILER_PROVIDER%"=="mingw" IF "%KDAPI_HOST_ARCH%"=="x86" IF "%KDAPI_TARGET_ARCH%"=="x64" (
  ECHO [ERROR] The build tool cannot compile to target architecture ^(%KDAPI_TARGET_ARCH%^)
  GOTO :_KDAPI_ERROR_EXIT
)
...
IF DEFINED KDAPI_BUILD_TOOLS_PATH IF "%KDAPI_COMPILER_PROVIDER%" NEQ "msvc" (
  SET KDAPI_COMPILER="%KDAPI_BUILD_TOOLS_PATH%\bin\%KDAPI_COMPILER_NAME%"
) ELSE (
...
@REM Check KDAPI_LINKER_NAME
IF DEFINED KDAPI_LINKER_NAME (
  IF "%KDAPI_COMPILER_PROVIDER%"=="mingw" (
    ECHO [BUILD] '%KDAPI_LINKER_NAME%' will be used as compiler instead of 'gcc.exe'
  ) ELSE IF "%KDAPI_COMPILER_PROVIDER%"=="llvm" (
    ECHO [BUILD] '%KDAPI_LINKER_NAME%' will be used as compiler instead of 'clang.exe'
  ) ELSE IF "%KDAPI_COMPILER_PROVIDER%"=="msvc" (
    ECHO [BUILD] '%KDAPI_LINKER_NAME%' will be used as compiler instead of 'link.exe'
  )
) ELSE (
  IF "%KDAPI_COMPILER_PROVIDER%"=="mingw" (
    SET KDAPI_LINKER_NAME=gcc.exe
  ) ELSE IF "%KDAPI_COMPILER_PROVIDER%"=="llvm" (
    SET KDAPI_LINKER_NAME=clang.exe
  ) ELSE IF "%KDAPI_COMPILER_PROVIDER%"=="msvc" (
    SET KDAPI_LINKER_NAME=link.exe
  )
)

IF DEFINED KDAPI_BUILD_TOOLS_PATH IF "%KDAPI_COMPILER_PROVIDER%" NEQ "msvc" (
  SET KDAPI_LINKER="%KDAPI_BUILD_TOOLS_PATH%\bin\%KDAPI_LINKER_NAME%"
) ELSE (
  SET KDAPI_LINKER=%KDAPI_LINKER_NAME%
)

ECHO [BUILD] Linker: %KDAPI_LINKER%
...
@REM Output binaries
SET KDAPI_EXEC_FILE=%KDAPI_OUTPUT_NAME%%KDAPI_BUILD_POSTFIX%.exe
